...
public class PostEventActivity extends Activity {
	...
	private boolean sendInfoDone = false, sendImageDone = false;
		...
		button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!cancel) {
					...
					PostToServerTask go = new PostToServerTask();
					UploadImageTask imgup = new UploadImageTask();
					go.setDone(new ResponseCallback() {
						public void processResponse(String response) {
							try {
								JSONObject jObject;
					...
					imgup.setDone(new ResponseCallback() {
						public void processResponse(String response) {
							sendImageDone = true;
						}
					});
					...
					try {
						...
						imgup.execute(imgBitmap, "upload.png", event_id);
						while (!sendInfoDone) {
							Thread.sleep(10);
						}
