...
class User extends ActiveRecord
{
    ...
    public static function authenticate ($email = null, $pass = null)
    {
        ...
        else if ($email != null)
        {
            ...
            if (!empty ($sess))
            {
                System::doMysql("UPDATE ".Session::getTableName()." SET last_activity = NOW() WHERE id=".$sess[0]->id);
    ...
    /**
     * returns a list of Online users (last activity < 5min ago)
     */
    public static function getOnline ()
    {
        return User::find("id IN (SELECT user_id FROM ".Session::getTableName()." WHERE last_activity > NOW() - INTERVAL 5 MINUTE);");
    }
